...
enum scheme_ref_type
  {
   ...
   // Ref found by tracing live Lisp objects.
   rt_trace,

   // Ref may or may not be valid.
   rt_trace_uncertain,
...
struct scheme_ref_info {
  ...
  // A pointer to where the ref is stored in memory.  If not NULL,
  // it's safe for the referenced value to be moved by the garbage
  // collector.
  chez_ptr *ref_ptr;

  enum scheme_ref_type type;
};
...
static int
scheme_ref_cmp (const void *p, const void *q)
{
  const struct scheme_ref_info *pp = p, *qq = q;
  return pointer_cmp (pp->ref, qq->ref);
}

static int
scheme_ref_ptr_cmp (const void *p, const void *q)
{
  const struct scheme_ref_info *pp = p, *qq = q;
  return pointer_cmp (pp->ref_ptr, qq->ref_ptr);
}
...
static bool
analyze_scheme_ref_impl (Lisp_Object ref,
                         Lisp_Object *ptr,
                         bool is_valid,
                         const char *label)
{
...
{
  return analyze_scheme_ref_impl (ref, NULL, false, label);
}
...
{
  return analyze_scheme_ref_impl (ref, NULL, true, label);
}

bool
analyze_scheme_ref_ptr (Lisp_Object *ptr, const char *label)
{
  return analyze_scheme_ref_impl (LISP_FALSE, ptr, true, label);
}
...
static bool
scheme_ref_dead_or_trace (const void *p)
{
  const struct scheme_ref_info *pp = p;
  switch (pp->type)
    {
...
static void
set_mark_bit (Lisp_Object obj, bool bit)
{
  switch (XTYPE (obj))
    {
    case Lisp_String:
      if (bit)
        MARK_STRING (XSTRING (obj));
      else
        UNMARK_STRING (XSTRING (obj));
      break;
    case Lisp_Vectorlike:
      if (bit)
        VECTOR_MARK (XVECTOR (obj));
      else
        VECTOR_UNMARK (XVECTOR (obj));
      break;
    case Lisp_Symbol:
...
static void
mark_lisp_refs_fun (void *arg, Lisp_Object *refs, ptrdiff_t n)
{
...
#else
#define mark_object_ptr(p) (mark_object(*p))
#endif
...
#ifdef HAVE_CHEZ_SCHEME
#undef SUSPICIOUS_OBJECT_CHECKING
#endif /* HAVE_CHEZ_SCHEME */
...
#ifdef HAVE_CHEZ_SCHEME
#define macro_XPNTR(a) ((void *) CHEZ (a))
#else
...
#define MALLOC_PROBE(size) ((void) 0)
...
static void
object_pre_gc (struct Scheme_Object_Header *soh)
{
  Lisp_Object obj = soh->scheme_obj;
  if (analyze_scheme_ref (obj, "object_pre_gc"))
    TRACEF ("soh = %p", soh);
...
static void
object_was_collected (Lisp_Object obj)
{
  struct Scheme_Object_Header *soh = get_scheme_object_header (obj);
  if (soh == NULL) return;
  analyze_scheme_ref (obj, "collected");
  soh->scheme_obj = LISP_FALSE;
}

static void *
scheme_allocate_vector (ptrdiff_t nbytes,
                        chez_ptr sym,
                        ptrdiff_t soh_offset)
{
  ...
  analyze_scheme_ref (UNCHEZ (vec), "scheme_allocate_vector");
  ...
  link_scheme_obj (soh, UNCHEZ (vec));
...
{
#ifdef HAVE_CHEZ_SCHEME
  return scheme_allocate_vector (sizeof (struct Lisp_String),
...
{
#ifdef HAVE_CHEZ_SCHEME
  struct Lisp_Vector *data = scheme_allocate_vector
...
#if HAVE_CHEZ_SCHEME
  memzero (v->contents, memlen * word_size);
#else
...
{
#ifdef HAVE_CHEZ_SCHEME
  struct Lisp_Misc_Any *data = scheme_allocate_vector
...
mark_finalizer_list (struct Lisp_Finalizer *head)
{
  for (struct Lisp_Finalizer *finalizer = head->next;
       ...
       finalizer = finalizer->next)
    {
      ...
      mark_object_ptr (&finalizer->function);
    }
}
...
{
#ifdef HAVE_CHEZ_SCHEME
  record_scheme_ref (obj, rt_trace_uncertain);
#else
#if USE_VALGRIND
  if (valgrind_p)
...
#ifdef HAVE_CHEZ_SCHEME
#define GC_POINTER_ALIGNMENT alignof (Lisp_Object)
#else
...
// TODO(jrw): Get rid of this.  Instead, record Scheme refs as they
// are copied into C code, along with the top of the stack.
// Periodically (in gc?) expire refs that are higher than the top of
// the current stack.
void
...
{
#ifdef HAVE_CHEZ_SCHEME
  return scheme_make_pure (Fmake_vector (make_number (len), Qnil));
#else /* not HAVE_CHEZ_SCHEME */
  Lisp_Object new;
...
#ifdef HAVE_CHEZ_SCHEME
#define BEGIN_LIST_ITERATION(list_var)                                  \
  Lisp_Object list_var##_start = UNCHEZ (chez_cons (chez_false, CHEZ (list_var))); \
  chez_lock_object (CHEZ (list_var##_start));                           \
  Lisp_Object list_var##_prev = list_var##_start
#define LIST_RECORD_PREV(list_var, new_prev)    \
  list_var##_prev = new_prev
#define LIST_UPDATE_PREV(list_var, new_list)    \
  chez_set_cdr (CHEZ (list_var##_prev), CHEZ (new_list))
#define END_LIST_ITERATION(list_var)                            \
  list_var = UNCHEZ (chez_cdr (CHEZ (list_var##_start)));       \
  chez_unlock_object (CHEZ (list_var##_start))
#else
#define BEGIN_LIST_ITERATION(list_var)          \
  Lisp_Object *list_var##_prev = &list_var
#define LIST_RECORD_PREV(list_var, new_prev)    \
  list_var##_prev = xcdr_addr (new_prev)
#define LIST_UPDATE_PREV(list_var, new_list)    \
  (*list_var##_prev = (new_list))
#define END_LIST_ITERATION(list_var) ((void)0)
#endif
  ...
  for (tail = list; CONSP (tail) && !CONS_MARKED_P (XCONS (tail));
	...
	tem = XCAR (tem);
      if (BUFFERP (tem) && !BUFFER_LIVE_P (XBUFFER (tem)))
        LIST_UPDATE_PREV (list, XCDR (tail));
      else
	{
#ifdef HAVE_CHEZ_SCHEME
          mark_object (tail);
	  ...
	  mark_object (XCAR (tail));
          LIST_RECORD_PREV (list, tail);
	}
    }
  END_LIST_ITERATION(list);
  mark_object (tail);
  return list;
}
...
before_scheme_gc (void)
{
  ...
  for (struct Scheme_Object_Header *soh =
  ...
  for (Lisp_Object *global_ptr = (Lisp_Object *) &globals;
       ...
       global_ptr++)
    {
      record_scheme_ref_ptr (global_ptr, rt_trace);
    }
  ...
  TRACEF ("mq start size: %lu", mark_queue.size);
...
after_scheme_gc (void)
{
  ...
  TRACEF ("remaining tracked refs: %d", (int)scheme_refs.size);
  ...
  /* while (true) */
  /*   { */
  /*     chez_ptr ref = chez_call0 (scheme_guardian); */
  /*     if (ref == chez_false) */
  /*       break; */
  /*     object_was_collected (UNCHEZ (ref)); */
  /*     ++guardian_count; */
  /*   } */
  ...
  struct Scheme_Object_Header **prev_soh_ptr =
    &first_scheme_object_header;
  chez_ptr prev_cell = scheme_object_list;
  chez_ptr cell = chez_cdr (prev_cell);
  ...
  while (soh)
    {
      ...
      if (chez_bwp_objectp (obj))
...
#endif
          if (analyze_scheme_ref_maybe_invalid (soh->scheme_obj, "unlinking"))
            TRACEF ("soh = %p", soh);
          ++unlinked_count;
        }
      else
        {
          ...
          analyze_scheme_ref (soh->scheme_obj, "still alive");
          ++still_linked_count;
          prev_soh_ptr = &soh->next;
          prev_cell = cell;
        }
