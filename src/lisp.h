...
typedef struct Lisp_Subr Lisp_Subr_Init;
#endif
...
#ifdef HAVE_CHEZ_SCHEME
  eassert (FLOATP (f));
  return chez_flonum_value (CHEZ (f));
#else /* not HAVE_CHEZ_SCHEME */
...
#define DEFUN(lname, fnname, sname, minargs, maxargs, intspec, doc)	\
  static struct Lisp_Subr sname =                                       \
    { { .s = { .size = PVEC_SUBR << PSEUDOVECTOR_AREA_BITS,             \
               .soh = { .scheme_obj = SCHEME_NIL,                       \
                        .next = NULL } } },                             \
      { .a ## maxargs = fnname },					\
      minargs, maxargs, lname, SCHEME_NIL, intspec, 0};                 \
   Lisp_Object fnname
#else /* not HAVE_CHEZ_SCHEME */
...
extern void defsubr (Lisp_Subr_Init *);
...
{
  struct Lisp_Vector *p = allocate_vector (size);
  Lisp_Object v;
  XSETVECTOR (v, p);
  return v;
}
